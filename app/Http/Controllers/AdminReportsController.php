...
class AdminReportsController extends Controller {
   ...
   public function admin_reports_check($name) {
      try {
         if (empty(Auth::id())) {
            return redirect()->route('login');
         }
         ...
         $user_role = Auth::user()->roles->first()->id;
         $user_id = Auth::id();
         $arrs = DB::table('tables')->where('periodicity',  $periodicity)->where('status', 0)->orWhere('status', 1)->orderBy('id', 'desc')->paginate(20);
