...
class DailyReportsController extends Controller {
    public function daily_reports() {
        try {
            ...
            $user_id = Auth::id();
            $arrs = DB::table('tables')->where('user_id', $user_id)->where('periodicity', '=', 1)->orWhere('periodicity', '=', 2)->orderBy('id', 'desc')->get();
                       foreach ($arrs as $key => $val) {
                foreach (json_decode($val->departments, true) as $depart) {
                    $fill_arrs_daily[$val->table_uuid][$depart] = DB::table('daily_reports')->where('table_uuid', $val->table_uuid)->where('user_dep', $depart)->value('json_val');
                }
            }
            foreach ($fill_arrs_daily as $table => $fill_table) {
                if (isset($table)) {
                    ...
                    foreach ($fill_table as $dep => $fill_dep) {
                        if (isset($fill_dep)) {
                            ...
                            $filled_arr_daily[$table] = intval(round($counter / ($cols + ($empty_deps * $cols)) * 100, 0, PHP_ROUND_HALF_UP));
                        }
                    }
                }
                }
                ...
                foreach (json_decode($arrs, true) as $key => $arr) {
                    foreach ($filled_arr_daily as $k => $fill) {
                        if ($arr['table_uuid'] == $k) {
