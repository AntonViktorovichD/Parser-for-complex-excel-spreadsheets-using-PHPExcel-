...
class AdminExportSheetController extends Controller {
    public function admin_export($row_uuid) {
        ...
        $json_values = json_decode(DB::table('report_values')->where('table_uuid', '=', $table_uuid)->pluck('json_val'), true);
        ...
        for ($j = 0; $j < count($json_values); $j++) {
            $json_val = json_decode($json_values[$j], true);
