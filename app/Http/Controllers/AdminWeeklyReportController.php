...
class AdminWeeklyReportController extends Controller {
    public function admin_weekly_reports() {
        try {
            $rv_ja = [];
            $user_names = [];
            $fill_arrs = [];
            ...
            foreach ($arrs as $key => $val) {
                foreach (json_decode($val->departments, true) as $depart) {
                    $fill_arrs[$val->table_uuid][$depart] = DB::table('weekly_reports')->where('table_uuid', $val->table_uuid)->where('user_dep', $depart)->value('json_val');
                }
            }
...
//            echo '</pre>';
            ...
            return view('admin_weekly_reports', ['arr' => $arrs, 'tableload' => '', 'arr_rows' => $arr_rows, 'user_id' => $user_id, 'user_role' => 'user_role', 'table_user' => $table_user, 'pages' => $arrs]);
    ...
    public function admin_weekly_update(Request $request) {
        ...
        try {
            ...
            return view('router', ['alert' => 'Запись успешно отредактирована', 'route' => '/admin_weekly_reports']);
        } catch (QueryException $e) {
            echo 'Ошибка: ' . $e->getMessage();
        }
    }
