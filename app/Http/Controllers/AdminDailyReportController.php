...
class AdminDailyReportController extends Controller {
   public function admin_daily_report($table_uuid) {
      try {
         ...
         $user_dep = [];
         $dep = [];
         foreach ($daily_reports as $key => $report_value) {
   ...
   public function admin_daily_report_date(Request $request) {
      try {
         ...
         asort($arrLR);
         $addRowArr = json_encode($arrLR, JSON_UNESCAPED_UNICODE);
         $daily_reports = json_decode($daily_reports, true);
         if (count($daily_reports) > 0) {
            foreach ($daily_reports as $i => $value) {
               $val = json_decode($daily_reports[$i]['json_val'], true);
               $key = explode('+', $daily_reports[$i]['row_uuid'] . '+');
            ...
            return view('admin_daily_report_date', compact('json', 'highest_row', 'highest_column_index', 'addRowArr', 'name', 'table_uuid', 'user_id', 'row_uuid', 'daily_report', 'user_dep', 'pattern', 'json_func', 'json_vals', 'dep'));
         } else {
